...
func NewGroomerRouter(env *bootstrap.Env, timeout time.Duration, db mongo.Database, router chi.Router) {
	ur := repository.NewUserRepository(db, domain.CollectionUser)
	gc := &controller.GroomerController{
		GroomerUsecase: usecase.NewGroomerUsecase(ur, timeout),
	}
	router.Get("/groomer", gc.Fetch)
}
